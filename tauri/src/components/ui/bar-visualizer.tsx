...
export function useAudioVolume(
  ...
  useEffect(() => {
    ...
    let lastUpdate = 0;
    const updateInterval = 1000 / 60; // 60 FPS for smoother updates
    ...
    const updateVolume = (timestamp: number) => {
      if (timestamp - lastUpdate >= updateInterval) {
        ...
        if (Math.abs(newVolume - volumeRef.current) > 0.005) {
...
const normalizeDb = (value: number) => {
  ...
  const minDb = -90; // Very sensitive floor to catch quiet mic input
  const maxDb = 0; // Max possible dB
    ...
    const updateVolume = (timestamp: number) => {
      if (timestamp - lastUpdate >= updateInterval) {
        ...
        for (let i = 0; i < chunks.length; i++) {
          if (Math.abs(chunks[i] - (bandsRef.current[i] ?? 0)) > 0.01) {
            hasChanged = true;
            break;
          }
        }
...
const BarVisualizerComponent = React.forwardRef<HTMLDivElement, BarVisualizerProps>(
  (
    ...
    // Audio processing
    // Note: loPass and hiPass are ARRAY INDICES into the frequency bin array, not Hz values
    // With fftSize 2048 at 48kHz, each bin is ~23Hz, so indices 5-300 cover ~115Hz to ~7kHz
    const realVolumeBands = useMultibandVolume(mediaStream, {
      bands: barCount,
      loPass: 5, // Start index (low frequencies)
      hiPass: 300, // End index (captures voice range)
    });
    ...
    useEffect(() => {
      ...
      const updateFakeVolume = (timestamp: number) => {
        if (timestamp - lastUpdate >= updateInterval) {
          ...
          let hasChanged = false;
          for (let i = 0; i < barCount; i++) {
            if (Math.abs(newBands[i] - (fakeVolumeBandsRef.current[i] ?? 0)) > 0.05) {
              hasChanged = true;
              break;
            }
          }
    ...
    const highlightedIndices = useBarAnimator(
      state,
      barCount,
      state === "connecting" ? 2000 / barCount
      : state === "thinking" ? 150
      : state === "listening" ? 500
      : 1000,
    );
    ...
    return (
      <div
        ...
        className={cn(
          ...
          "bg-slate-100 h-32 w-full overflow-hidden rounded-lg p-4 dark:bg-slate-800",
          className,
        )}
        style={{
          ...style,
      ...
      >
        {volumeBands.map((volume, index) => {
          const heightPct = Math.min(
            maxHeight,
            Math.max(minHeight, volume * 300 + 10), // Aggressive boost: 3x multiplier on 0-1 range
          );
...
const Bar = React.memo<{
  ...
  <div
    ...
    className={cn(
      ...
      "rounded-full",
      "bg-slate-200 data-[highlighted=true]:bg-slate-900 dark:bg-slate-800 dark:data-[highlighted=true]:bg-slate-50",
      state === "speaking" && "bg-slate-900 dark:bg-slate-50",
