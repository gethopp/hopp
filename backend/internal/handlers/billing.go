...
func NewBillingHandler(db *gorm.DB, config *config.Config, jwtIssuer common.JWTIssuer) *BillingHandler {
	// Set Stripe API key
...
// getAuthenticatedUserFromJWT extracts the user from JWT token
// This is duplicate with handlers.go util
// TODO: Refactor to use a common function/props and avoid duplication
// TODO: Maybe merge with handlers to avoid duplication and share logggers and stuff that comes from echo? Echo loggers could be shared also as param to the function?!!
func (bh *BillingHandler) getAuthenticatedUserFromJWT(c echo.Context) (*models.User, error) {
	email, err := bh.JwtIssuer.GetUserEmail(c)
	if err != nil {
		return nil, err
	}

	// Fetch user from database
	user, err := models.GetUserByEmail(bh.DB, email)
	if err != nil {
		return nil, err
	}
...
func (bh *BillingHandler) CreateCheckoutSession(c echo.Context) error {
	user, err := bh.getAuthenticatedUserFromJWT(c)
	if err != nil {
...
func (bh *BillingHandler) GetSubscriptionStatus(c echo.Context) error {
	...
	// No subscription there are on free tier
...
func (bh *BillingHandler) handleSubscriptionUpdated(c echo.Context, event stripe.Event) error {
	...
	if subscription.CancelAtPeriodEnd {
		...
		if adminUser != nil {
			c.Logger().Infof("Sending email to admin user for acknowledgement: %s", adminUser.Email)
		}
	}
...
func (bh *BillingHandler) handleCheckoutSessionCompleted(c echo.Context, event stripe.Event) error {
	...
	// TODO: add email + telegram hook for notifications

	// Save in transaction
	return bh.DB.Transaction(func(tx *gorm.DB) error {
